...
namespace SpiderEye.Linux
{
    ...
    internal class GtkWebview : IWebview
    {
        ...
        {
            ColorTools.ParseHex(color, out byte r, out byte g, out byte b);
            var rgba = new RGBA
            {
                Red = r / 255F,
                Green = g / 255F,
        ...
        private static void LocalFileSchemeCallback(URISchemeRequest request, string pathPrefix, string directory)
        {
            ...
            var file = Functions.FileNewForPath(fullFilePath);
        ...
        {
            /* TODO check does this work? var quark = GLib.Functions.FileErrorQuark();
            Error.NewLiteral(quark, errorCode, null);
            var error = new GError(domain, errorCode, IntPtr.Zero);*/
            request.FinishError(new Error
            {
                Code = errorCode,
            });
        }
